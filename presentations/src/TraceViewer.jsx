...
function TraceViewer() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        document.title = `Trace - ${lectureName}`;
      } catch (error) {
        console.error(error);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      if (!event.shiftKey && (event.key === 'ArrowRight' || event.key === 'l')) {
        ...
        gotoTrace({lectureName, navigate});
      } else {
        return;
      }
  ...
  // Not ready
  if (!lectureName) {
    return <div>No lecture name provided</div>;
  }
  if (error) {
...
function gotoTrace({lectureName, navigate}) {
  const newLectureName = prompt("Enter the lecture name", lectureName);
  if (newLectureName) {
    updateUrlParams({ lecture: newLectureName, source: null, line: null, step: null }, navigate);
  }
}
